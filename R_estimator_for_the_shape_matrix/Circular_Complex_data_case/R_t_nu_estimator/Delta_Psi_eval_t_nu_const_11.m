function [Delta_T_1, KK_1] = Delta_Psi_eval_t_nu_const_11(y, T, nu, D_n)
...
% Evaluation of the matrix K_V for the constraint 11
inv_sr_T2 = kron(inv_sr_T.',inv_sr_T);
I_m = eye(m);
J_m_per = eye(m^2) - I_m(:)*I_m(:).'/m;
I_bar = eye(m^2);
I_bar(1,:) = [];
K_V_1 = I_bar * inv_sr_T2*J_m_per;
...
Delta_T_1 = K_V_1*Score_appo/sqrt(n);
KK_1 = (K_V_1*K_V_1');
end
