...
vec_V_S = V_S(:);
Inv_V_S = inv(V_S);
P_S = I_m2-(1/m)*V_S(:)*(I_m(:).');
T_1_appo = vec_I_m.'*D_m;
...
for il=1:Nl
    ...
    MSE_SCM = zeros(m^2,m^2);
    MSE_Ty = zeros(m^2,m^2);
    MSE_R_vdW = zeros(m^2,m^2);
    MSE_R_t_nu = zeros(m^2,m^2);
    ...
    parfor ins=1:Ns
        ...
        [V_RvdW, ~] = R_vdW_est_const_trace(y, V_Ty, perturbation_par);
        ...
        [V_Rt, ~] = R_t_nu_est_const_trace(y, V_Ty, nu_par, perturbation_par);
    ...
    SCR_Bound(il) = norm(FULL_SCRB,'fro');
    CR_Bound(il) = norm(FULL_CRB,'fro');
...
title('Estimation of $\underline{\mathrm{vecs}}(\mathbf{V}_S)$ with $S(\mathbf{\Sigma})=\mathrm{trace}(\mathbf{\Sigma})/m$','interpreter','latex')

save const_trace.mat